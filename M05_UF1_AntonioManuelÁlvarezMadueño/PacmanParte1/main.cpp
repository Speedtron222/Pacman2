...
/// </summary>
void Setup(unsigned short &enemyNumber);
/// <summary>
/// Handles the inputs
/// </summary>
void Input();
...
/// Draws the screen
/// </summary>
void Draw(float &c, int &cs, unsigned short& enemyNumber);
...
int main()
{
    ...
    while (info.run)
    {
        Input();
        Logic();
        Draw(info.counter, info.currentSpawn, enemyNumber);
    }
}
...
void Draw(float &c, int& cs, unsigned short &enemyNumber)
{
    ...
    //}
    c += TimeManager::getInstance().deltaTime;
    if (c > 10 && enemyNumber > 0) {
        info.enemigos.push_back(Enemy(info.pacman_map.spawn_enemy));
        c = 0;
        enemyNumber--;
    }
